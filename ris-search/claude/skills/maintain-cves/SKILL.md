---
name: maintain-cves
description: Audit existing CVE pins, overrides, and ignores across the project and remove any that are no longer needed because the vulnerability has been fixed upstream. Covers backend (Gradle), frontend (pnpm overrides), frontend image (.trivyignore), and api-docs (pnpm overrides and .trivyignore).
---
...
### API docs overrides — `api-docs/pnpm-workspace.yaml`

All entries in the `overrides:` block are candidates (same logic as frontend overrides).

### API docs image ignores — `api-docs/.trivyignore`

All CVE IDs listed here (same logic as frontend image ignores).
...
1. Remove **all** entries from the `overrides:` block in `api-docs/pnpm-workspace.yaml`.
2. Run:
...
### API docs image ignores

1. Remove **all** entries from `api-docs/.trivyignore`.
2. Build the api-docs image:
